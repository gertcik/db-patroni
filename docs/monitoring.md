...
На **физической реплике** — задержка по времени (когда был последний replayed транзакционный commit):

```sql
-- задержка репликации в секундах/минутах
SELECT now() - pg_last_xact_replay_timestamp() AS replication_delay;
```

`pg_last_xact_replay_timestamp()` возвращает время последней транзакции, применённой через replay. Если значение больше нескольких секунд — реплика отстаёт от мастера.
